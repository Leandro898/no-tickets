    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="relative bg-white border border-gray-100 rounded-2xl shadow-md flex flex-col justify-between min-h-[270px] p-0 overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div class="pt-8 px-7 pb-1">
                {{-- Cambiado el texto del badge a "Entrada: Nombre" --}}
            ...
            {{-- Datos: Stock y qty --}}
            {{-- Modificamos esta sección para ser más informativa y clara --}}
            <div class="flex flex-row justify-around px-7 py-2 text-gray-600 border-t border-gray-50">
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-base font-semibold">{{ $e->stock_inicial - $e->stock_vendido }}</span>
                </div>
                <div class="flex flex-col items-center">
                    <span class="text-xs font-bold uppercase">Vendidas</span>
                    {{-- Mostramos las entradas vendidas --}}
                    <span class="text-base font-semibold">{{ $e->stock_vendido ?? '0' }}</span>
                </div>
                <div class="flex flex-col items-center">
            ...
            {{-- Mejoramos la etiqueta de estado --}}
            <div class="flex justify-between items-center px-7 py-3 bg-gray-50 border-t border-gray-100 rounded-b-2xl">
                {{-- Aquí puedes agregar lógica para mostrar el estado real: "Configurado" o "Sin stock" --}}
                <div class="flex items-center gap-2 text-{{ ($e->stock_inicial > 0) ? 'green' : 'red' }}-600">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <circle cx="10" cy="10" r="10" />
                    </svg>
                    <span class="text-xs font-semibold">
                        @if ($e->stock_inicial > 0)
                        Disponible
                        @else
                        Sin stock
                        @endif
                ...
                {{-- La cantidad vendida ya se muestra arriba, podemos usar este espacio para otra info --}}
                <span class="text-xs text-gray-500 font-semibold">{{ $e->stock_vendido ?? 0 }} / {{ $e->stock_inicial ?? 'N/A' }} vendidas</span>
            </div>
        </div>
        @empty
